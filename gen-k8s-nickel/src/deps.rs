...
fn _get_def_deps(
  ...
  let mut imports =
    deps_graph
      .get(definition_name)
      .map_or(Vec::<String>::new(), |deps| {
        deps
          .iter()
          .sorted()
          .flat_map(|d| _get_def_deps(deps_graph, d))
          .collect()
      });
...
fn get_correct_order_of_in_statements(
  ...
  definitions_without_dependency
    .iter()
    .flat_map(|def_name| _get_all_deps(deps_graph, def_name))
    .unique()
    .collect()
}
...
mod tests {
  ...
  fn _create_test_graph() -> HashMap<String, HashSet<String>> {
    ...
    dpg.insert(
      "A".to_string(),
      HashSet::from_iter(std::array::IntoIter::new([
        "C".to_string(),
        "D".to_string(),
        "E".to_string(),
      ])),
    ...
    dpg.insert(
      "C".to_string(),
      HashSet::from_iter(std::array::IntoIter::new(["G".to_string()])),
    );
    dpg.insert(
      "D".to_string(),
      HashSet::from_iter(std::array::IntoIter::new([])),
    ...
    dpg.insert(
      "I".to_string(),
      HashSet::from_iter(std::array::IntoIter::new(["N".to_string()])),
    );
    dpg.insert(
      "J".to_string(),
      HashSet::from_iter(std::array::IntoIter::new(["M".to_string()])),
    );
    dpg.insert(
      "K".to_string(),
      HashSet::from_iter(std::array::IntoIter::new(["M".to_string()])),
