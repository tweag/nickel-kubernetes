...
fn _to_ref_def(name: &str, indent: usize, ref_path: &Option<String>) -> String {
  ref_path.as_ref().map_or(
    format!("<sensible_warning:{}>", name),
    |r| format!("{}{} | {}", format_args!("{:>1$}", "", indent), name, k8s_ref_to_ncl_id(r.as_str()))
  )
}
...
fn _to_int_def(name: &str, indent: usize) -> String {
  format!("{}{} | #(nums.Int)", format_args!("{:>1$}", "", indent), name)
}

fn _to_list_def(name: &str, indent: usize, items: &Option<Box<openapi::Schema>>) -> String {
  let item_type = items.as_ref().map_or(format!("<sensible_warning:{}>", name), |i| {
    i.ref_path.as_ref().map_or(
      format!("<sensible_warning:{}>", name),
      |r| k8s_ref_to_ncl_id(r)
    )
  }); 
  format!("{}{} | List {}", format_args!("{:>1$}", "", indent), name, item_type)
}

fn _to_custom_type_def(name: &str, indent: usize, properties: &Option<std::collections::BTreeMap<String, openapi::Schema>>) -> String {
  if properties.as_ref().is_none() {
    format!("{}{} | #(utils.DictStrStr)", format_args!("{:>1$}", "", indent), name)
  } else {
    let props_defs = properties.as_ref().map_or(
      format!("<sensible_warning:{}>", name), 
      |p| {
        p.into_iter().map(|p| {
          let (n, s) = p;
    ...
    format!("{}{} = {{\n{}\n{}}}", format_args!("{:>1$}", "", indent), name, props_defs, format_args!("{:>1$}", "", indent))
  }
}
